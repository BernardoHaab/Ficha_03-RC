...
int main(int argc, char **argv)
{
  ...
  // Associa o socket a um endereço IP e um porto
  if (bind(
          serverSocketFD,
  ...
  debugMessage(stdout, OK, "Successfully binded to " RED "%s" RESET ":" GREEN "%d" RESET "\n",
               inet_ntop(
                   AF_INET,
  ...
  // Inicia a escuta de conexões (aceita até 5 conexões)
  ...
  loop
  {
    ...
    // Aceita a conexão com o cliente TCP
    if ((clientSocketFD = accept(
             serverSocketFD,
             (struct sockaddr *)&clientIPAddress,
             (socklen_t *)&clientIPAddressSize)) <= 0)
    {
      continue;
    }
