...
#define DOMAIN_NOT_FOUND \
  ...
  {
    // Criar mensagem de resposta com erro
  ...
  while (!found && (nread = getline(&line, &length, domainFile)) != -1)
  {
    ...
    {
      // Cria mensagem de resposta com dominio
      found = true;
...
#define IP_NOT_FOUND \
  ...
  if (argument == NULL || argument[0] == '\0')
  {
    // Cria mensagem de resposta com erro
    sprintf(response, "Argument not provided\n");
    return;
  }
  ...
  while (!found && (nread = getline(&line, &length, domainFile)) != -1)
  {
    ...
    {
      // Cria mensagem de resposta com ip do dominio
      found = true;
  ...
  if (!found)
  {
    // Cria mensagem de resposta com ip nao encontrado
    sprintf(
        response,
        IP_NOT_FOUND,
        argument);
